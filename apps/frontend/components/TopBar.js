    ...
    >
      <div className="text-left mb-4">
        ...
        <h1 className="text-black text-lg">
          Config: <span className="font-semibold">{(monitoredPVs.has(configName) ? monitoredPVs.get(configName)[0] : "UNKNOWN")}</span>
        </h1>
      </div>
      <div
        id="inst_name"
      ...
      >
        <h2
          ...
          ${getStatusColor(monitoredPVs.has(runStateStr) ? monitoredPVs.get(runStateStr)[0] : "UNKNOWN")} ${getForegroundColor(
...
function getMonitoredPVs(index, monitoredPVs) {
  let dom = [];
  for (const [pv, pvarr] of monitoredPVs.entries()) {
    if (pvarr[1] == index) {
      dom.push(
        <tr
          id={pv}
